...
//  @transient lazy val sc: SparkContext = new SparkContext(conf)
  @transient val spark : SparkSession = SparkSession.builder.master("local[4]").appName("stackoverflow").getOrCreate()
...
//  sc.setLogLevel("ERROR")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val lines   = sc.textFile("gs://ruiwen_test_storage/bigdata/stackoverflow.csv")
...
class StackOverflow extends StackOverflowInterface with Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = {
    val q:RDD[(QID, Question)] = postings.filter(p => p.postingType == 1).map(p => (p.id, p))
    val a:RDD[(QID, Answer)] = postings.filter(p => p.postingType == 2).map(p => (p.parentId.get, p))
    q.join(a).groupByKey()
  }
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] = {
    ...
    scored.map(s => (firstLangInTag(s._1.tags,langs).get * langSpread:LangIndex,s._2))
  }
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] = {
    ...
    vectors.map(v => (findClosest(v,means),v)).groupByKey().mapValues(averageVectors).collect().foreach(x=>newMeans.update(x._1,x._2))
